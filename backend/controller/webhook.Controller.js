  ...
  //   if (!imageUrl) {
  //     console.warn("⚠️ No image found in sheet, skipping image send");
  //     return this.sendText(to, caption); // image nahi → sirf text
  //   }
...
static async sendImage(to, imageUrl, caption = '') {
  ...
  return await this.sendMessage(to, {
    ...
    image: {
      link: imageUrl,
      caption: caption.substring(0, 3000) // 👈 THIS IS THE "CARD"
    }
  });
}
  ...
  static getSafeImage(product) {
    if (!product.images) return null;

    // 🔹 Case 1: image already array
    if (Array.isArray(product.images) && product.images.length > 0) {
      return product.images[0];
    }

    // 🔹 Case 2: image is string (Google Sheet)
      ...
      static async sendProductCards(to, products, totalFound) {
        try {
          ...
          for (let i = 0; i < products.length; i++) {
            ...
            const imageUrl = WhatsAppService.getSafeImage(product);
  ...
  return products.map((product) => {
    ...
    // ✅ FIX 2: sizes → STRING → NUMBER ARRAY
    const sizes =
      typeof product.sizes === "string"
        ? product.sizes.split(",").map(s => Number(s.trim()))
    ...
    return {
      ...
      colors: product.colors || ["Black", "Blue"],
      features: product.features || ["Lightweight", "Comfortable"],
      inStock: true,
      deliveryDays: 3,
      material: "Breathable Mesh",
      warranty: "6 Months",
      productCode: `SAR-${product.type}-${product.id}`
    };
  });
  ...
  (async () => {
    try {
      const sheetProducts = await readProductsFromSheet();
      enhancedProducts = enhanceProducts(sheetProducts);
      console.log("✅ Products loaded from Google Sheet:", enhancedProducts.length);
    } catch (err) {
      console.error("❌ Failed to load products from sheet:", err.message);
    }
  })();
  ...
  async function handleSizeAndShowProducts(phone, text, state) {
    // 🛑 STEP GUARD – duplicate webhook call stop
    if (state.step !== "SIZE") return;
  ...
  let matchedProducts = enhancedProducts.filter(p => {
    // 🛡️ Safety checks
    if (!p.type || !p.price || !Array.isArray(p.sizes)) return false;

    const normalizedType = String(p.type).toUpperCase();

    // 🔁 TYPE MATCH FIX (Sheet text → Code category)
    if (state.type === "SPORTS" && !normalizedType.includes("SPORT")) return false;
    if (state.type === "CASUAL" && !normalizedType.includes("CASUAL")) return false;
    if (state.type === "FORMAL" && !normalizedType.includes("FORMAL")) return false;

    // 💰 PRICE FILTER
    if (Number(p.price) < state.min || Number(p.price) > state.max) return false;
    ...
    // ❌ NO SHOES FOUND (exact match bhi nahi)
    ...
    // ✅ SEND FOUND MESSAGE (ONLY ONCE)
    await WhatsAppService.sendText(
      phone,
      `🎉 *Found ${productsToShow.length} matching shoes!*\n\n` +
      `Now showing best options:`
    );
    ...
    selectedShoes: [
      {
        ...
        imageUrl: WhatsAppService.getSafeImage(state.selectedProduct),
      }
    ],
